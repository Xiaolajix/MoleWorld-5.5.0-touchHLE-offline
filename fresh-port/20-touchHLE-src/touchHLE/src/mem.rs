...
impl Mem {
    ...
    pub fn alloc_in_heap(
        ...
        // A request for an absurdly large block almost always means the guest
        // computed a bogus size (e.g. an underflow to a near-4GiB value, as
        // MoleWorld's +[CryptUtils doCipher:...] does on empty/short input:
        // 0xfffffc20). Real malloc returns NULL for an impossible request rather
        // than aborting; mirror that so the guest's own NULL-check path runs,
        // instead of killing the emulator.
        const BOGUS_ALLOC_THRESHOLD: GuestUSize = 0x4000_0000; // 1 GiB
        if size >= BOGUS_ALLOC_THRESHOLD {
            log!(
                "Warning: refusing absurd heap allocation of {size:#x} bytes; returning NULL (malloc-failure semantics)."
            );
            return Ptr::null();
        }
