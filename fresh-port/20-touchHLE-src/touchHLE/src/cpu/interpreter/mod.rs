...
///  - `option_env!` 在编译期读取,rustc 会把这个环境变量记进依赖信息,改了它会触发重编。
///  - 关闭时 `if INTERP_TRACE { … }` 整块在编译期被消除;INTERP-UNIMPL 的 Error 返回、
///    PC 落进高地址栈区的兜底停机都不受这个开关影响。
///  - [iOS⇄main 合并 2026-09-24] iOS 分支(8b75bed)独立做过同一件事,用的是 cargo feature
///    `interp_debug`(`--features static,cpu_interpreter,interp_debug`);两种开法都保留,任一开启即生效。
const INTERP_TRACE: bool = cfg!(any(feature = "interp_debug", debug_assertions))
    || option_env!("MOLE_INTERP_TRACE").is_some();

/// [iOS⇄main 合并 2026-09-24] 轻量心跳 / 卡死探针开关(iOS 分支的 `interp_hb` feature):
/// 只 +1 计数 +1 分支(~1.05x),不写 trace 环、不做 DERAIL 检测。记账全开(INTERP_TRACE)时一并开启,
/// 与 iOS 侧原来 `#[cfg(any(interp_debug, interp_hb, debug_assertions))]` 的范围一致。
const INTERP_HB: bool = INTERP_TRACE || cfg!(feature = "interp_hb");
...
impl InterpreterCpu {
    ...
    pub fn dump_trace(&self) {
        ...
        // 现场)照常调用,不受影响。
        if !INTERP_TRACE {
            echo!("[TRACE] 解释器指令 trace 未启用(release 构建需带编译期环境变量 MOLE_INTERP_TRACE=1 或 --features interp_debug 重编)");
            return;
        }
    ...
    fn step_one(&mut self, mem: &mut Mem) -> CpuState {
        ...
        // [iOS⇄main 合并 2026-09-24] 原 #[cfg(any(interp_debug, interp_hb, debug_assertions))] 改为编译期
        // 常量 INTERP_HB(见文件顶部),以便 main 的 MOLE_INTERP_TRACE 环境变量开关也能打开;关闭时整块编译期消除。
        if INTERP_HB {
