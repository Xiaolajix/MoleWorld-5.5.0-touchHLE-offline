...
fn substitute_classes(
    ...
    if !(name.starts_with("AdMob")
        ...
        // Security.framework kSec* constants and null-derefs
        // (+[TDGAKeyChain getKeychainQuery:]), same failure class as
        // TMA_SSKeychain. We fake ONLY the keychain helper, not the whole TDGA*
        // namespace: faking with too broad a prefix (e.g. all "TalkingData"/
        // "TDGA") breaks classes that participate in the class hierarchy as a
        // superclass, causing "found FakeClass, expected ClassHostObject".
        || name == "TDGAKeyChain"
        || name == "TDGASecureUDID"
        || name == "TDGAOpenUDID"
        // TDGAUtility: TalkingData device-info gatherer (getCpuFrequency/
        // getResolution/getMacAddress/md5String/isAppCracked/getInstallationTime
        // …). Leaf utility class (42 class methods, 0 subclasses), pure
        // analytics. Several of its methods crash under HLE (null strings into
        // to_rust_string, etc). Faking it skips analytics cleanly.
        || name == "TDGAUtility"
        // iRate: "please rate this app" nag library. Not gameplay. Its +load
        // runs -[iRate localizedStringForKey:] which needs NSBundle bundle
        // loading we don't support; faking the class skips it entirely.
        || name == "iRate"
        // TMA_SSKeychain: TaoMee Analytics' bundled SSKeychain, used to stash a
        // device/account identifier in the iOS keychain for analytics. We have
        // no real keychain (and want no analytics, offline). Faking it makes
        ...
        // is on the critical path *before* the game sets up its cocos2d renderer
        // (-[GameViewController setupCocosWithWindow:]). Faking it (no-op) lets
        // boot proceed straight to renderer setup. Fully aligned with "offline,
        // no networking/analytics".
        || name == "TaomeeAnalytics"
        ...
        // (合并注:上面 7 个精确类名来自 iOS 分支 3e9a80a 启动提速;下面按前缀 fake 的广告 SDK 来自 main
        //  的「登录进村跨游戏广告」修复。两组互不重叠,都保留。)
