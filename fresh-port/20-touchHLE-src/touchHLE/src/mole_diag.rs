...
// [合并注 2026-09-24] iOS 上还有真机取景器 ios_shot 在用(见下),所以两个平台都要导入。
...
///    debug::write_ppm 遇到不可写路径直接 unwrap panic。iOS 上本函数不再做 MOLE_DIAG 周期截帧。
///    [合并注 2026-09-24] iOS 上只保留 iOS 分支 8bc7046 的「真机取景器」ios_shot:文件触发
///    (Documents/mole_shot_now),空闲时每 60 帧一次 metadata 探测,不放触发文件就永不 glReadPixels。
